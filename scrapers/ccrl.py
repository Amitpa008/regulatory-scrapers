...
CCRL_SOURCE_LABEL = "CCRL"
CCRL_CIRCULARS_URL = "https://www.ccrl.co.in/circulars.html"
...
CCRL_LINK_PREFIX = "https://www.ccrl.co.in/"
...
class CCRLScraper(BaseScraper):
    source = "ccrl"
    regulator = CCRL_SOURCE_LABEL
    ...
    def fetch_index(self, from_date: date, to_date: date) -> Any:
        ...
        return self.fetch_page_html(CCRL_CIRCULARS_URL)

    def parse_listing(self, response: Any) -> list[dict[str, Any]]:
        records = self.parse_circular_records(str(response), CCRL_CIRCULARS_URL)
        return [
            {
    ...
    def discover_circular_range(self, url: str) -> dict[str, Any]:
        ...
        if not records:
            raise RuntimeError("CCRL circular page returned zero rows")
        ...
        earliest_records = sorted(records, key=lambda row: (row.date, row.circular_no, row.subject))[:10]
        result = {
            "working_route": url,
            ...
            "limitation": "Single raw HTML table currently exposes the full accessible CCRL archive; one malformed duplicate row in the live HTML is skipped when its date cannot be parsed.",
        }
    ...
    def scrape_listing_url(
        ...
        *,
        ...
        chunks = [CCRLChunk(index=1, label="circulars_page")]
        ...
        written_records: list[CCRLRecord] = []
        duplicates_skipped = 0
        for chunk in target_chunks:
            del chunk
            fresh_records: list[CCRLRecord] = []
    ...
    def validate_export(self, file_path: str | Path) -> dict[str, Any]:
        ...
        report_path = file_path.parent / "ccrl_circulars_validation_report.json"
        year_counts_path = file_path.parent / "ccrl_circulars_year_counts.csv"
        department_counts_path = file_path.parent / "ccrl_circulars_department_counts.csv"
        ...
        with open(file_path, "r", newline="", encoding="utf-8") as file_obj:
            ...
            try:
                ...
                raise RuntimeError(f"CCRL export is empty: {file_path}") from exc
            ...
            for row_number, row in enumerate(reader, start=2):
                ...
                if not link:
                    ...
                    if lowered.endswith(".pdf"):
                        ...
                        other_links += 1
                    if not link.startswith(CCRL_LINK_PREFIX):
    ...
    def parse_circular_records(self, html: str, source_url: str) -> list[CCRLRecord]:
        soup = BeautifulSoup(html, "html.parser")
        table = soup.find("table")
        if table is None:
            return []
        scraped_at = datetime.now(timezone.utc).isoformat()
        records: list[CCRLRecord] = []
        for row in table.find_all("tr")[1:]:
            cells = row.find_all("td")
            if len(cells) < 5:
                continue
            raw_date = normalize_text(cells[0].get_text(" ", strip=True)) or ""
            parsed_date = self.parse_ccrl_date(raw_date)
            circular_no = normalize_text(cells[1].get_text(" ", strip=True)) or ""
            department = normalize_text(cells[2].get_text(" ", strip=True)) or ""
            subject = normalize_text(cells[3].get_text(" ", strip=True)) or ""
            anchor = cells[4].find("a", href=True)
            link = self.normalize_ccrl_link(anchor.get("href"), source_url) if anchor else ""
    ...
    def parse_ccrl_date(self, raw_value: str) -> str:
        try:
    ...
    def normalize_ccrl_link(self, raw_value: Optional[str], source_url: str) -> str:
        value = normalize_text(raw_value) or ""
        if not value:
            return ""
        absolute = urljoin(source_url, value.replace(" ", "%20"))
        absolute = absolute.replace("https://ccrl.co.in/", CCRL_LINK_PREFIX)
        return absolute
    ...
    def record_dedup_key(self, record: CCRLRecord) -> tuple[str, str, str, str] | tuple[str, str, str]:
    ...
    def count_by_department(self, records: list[CCRLRecord]) -> dict[str, int]:
        counts: dict[str, int] = {}
        for record in records:
    ...
    def count_link_types(self, records: list[CCRLRecord]) -> dict[str, int]:
    ...
    def load_existing_output_records(self, out_path: str | Path) -> list[CCRLRecord]:
        out_path = Path(out_path)
    ...
    def append_output(self, records: list[CCRLRecord], out_path: str | Path, *, include_department: bool = False) -> None:
        out_path = Path(out_path)
    ...
    def write_output(self, records: list[CCRLRecord], out_path: str | Path, *, include_department: bool = False) -> None:
        out_path = Path(out_path)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        rows = [self.record_to_output_row(item, include_department=include_department) for item in records]
        fieldnames = ENRICHED_OUTPUT_HEADERS if include_department else EXPECTED_OUTPUT_HEADERS
    ...
    def record_to_output_row(self, record: CCRLRecord, *, include_department: bool = False) -> dict[str, str]:
        row = {
            "date": record.date,
    ...
    def append_metadata_sidecar(self, records: list[CCRLRecord], out_path: str | Path) -> None:
    ...
    def write_metadata_sidecar(self, records: list[CCRLRecord], out_path: str | Path) -> None:
        sidecar_path = self.metadata_sidecar_path(out_path)
        sidecar_path.write_text(json.dumps([asdict(item) for item in records], ensure_ascii=False, indent=2), encoding="utf-8")

    def load_metadata_sidecar(self, out_path: str | Path) -> list[CCRLRecord]:
        sidecar_path = self.metadata_sidecar_path(out_path)
        if not sidecar_path.exists():
            return []
        return [CCRLRecord(**item) for item in json.loads(sidecar_path.read_text(encoding="utf-8"))]
    ...
    def load_checkpoint(self, checkpoint_path: str | Path) -> CCRLCheckpoint:
        return CCRLCheckpoint(**json.loads(Path(checkpoint_path).read_text(encoding="utf-8")))

    def save_checkpoint(self, checkpoint_path: str | Path, checkpoint: CCRLCheckpoint) -> None:
